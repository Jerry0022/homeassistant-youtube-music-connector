...
class YtmcSearchPlay extends HTMLElement {
  ...
  _toggleTarget(entityId) {
    if (this._selectedTargets.has(entityId)) {
    ...
    // Also set the first selected as the connector's active target
    const first = this._selectedTargets.values().next().value;
    if (first) {
      this._hass.callService("media_player", "select_source", { entity_id: this._entityId, source: first });
    }
    this._renderSig = "";
    this._tryRender();
  }
